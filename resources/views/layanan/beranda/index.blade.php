<x-layout :title="$title ?? 'Beranda Layanan'">
        ...
        <div class="rounded-2xl overflow-hidden shadow-sm border border-slate-200 bg-white mb-6">
            <div class="px-4 sm:px-6 py-5 bg-linear-to-r from-indigo-600 to-sky-500 text-white">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div>
                        <h1 class="text-lg sm:text-xl font-semibold">Beranda Layanan</h1>
                        <p class="text-xs sm:text-sm text-indigo-100/90">Ringkasan pengaduan pada layanan Anda.</p>
                    </div>
                    <a href="{{ route('layanan.pengaduan.index', ['jenis_layanan_id' => auth()->user()->layanan_id]) }}"
            ...
            {{-- CARDS --}}
            <div class="px-4 sm:px-6 py-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-3">
                ...
                {{-- <div class="p-4 border rounded-2xl bg-indigo-50 border-indigo-200">
                    ...
                    <p class="text-2xl font-semibold text-indigo-700">{{ $menunggu }}</p>
                </div> --}}
                        ...
                        <table class="min-w-[720px] md:min-w-full text-sm">
                            <thead class="bg-slate-50 text-slate-600 border-b">
                                <tr>
                                    ...
                                    <th class="text-left py-3 px-3">UPT</th>
                            ...
                            <tbody>
                                @forelse($recent as $p)
                                    <tr class="border-b last:border-0">
                                        ...
                                        <td class="py-3 px-3">{{ optional($p->unit)->name ?: '—' }}</td>
                                        ...
                                        <td class="py-3 px-3">
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-1 rounded-full border text-xs 
    {{ $p->status_label === 'Diproses oleh user layanan '
