...
<x-layout :title="$title ?? 'Disposisi Pengaduan'">
        ...
        <div class="rounded-2xl overflow-hidden shadow-sm border border-slate-200 bg-white">
            <div class="px-6 py-5 bg-linear-to-r from-indigo-600 to-sky-500 text-white">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <h1 class="text-lg font-semibold">Disposisi Pengaduan</h1>
                        <p class="text-xs text-indigo-100/90">Kelola proses disposisi, tarik kembali, dan penutupan
                            pengaduan.</p>
                    </div>
                </div>
            </div>
                            ...
                            @forelse ($items as $row)
                                ...
                                <tr class="hover:bg-slate-50 transition" x-data="{ openDispo: false, openJawab: false, openDetail: false }">
                                    ...
                                    <td class="border border-slate-200 px-4 py-3 whitespace-nowrap">
                                        <span
                                            class="inline-flex items-center gap-1.5 px-2 py-0.5 text-xs font-medium rounded-full border {{ $row->status_badge_class }}">
                                    ...
                                    <td class="border border-slate-200 px-4 py-3 text-left whitespace-nowrap">
                                        <div class="inline-flex items-center gap-2">
                                            ...
                                            <a href="{{ route('upt.disposisi.show', $row) }}"
                                        ...
                                        {{-- MODAL DISPOSISI (tidak diubah)… --}}
                                        <div class="fixed inset-0 z-60 bg-black/40 backdrop-blur-sm" x-show="openDispo"
                                            x-transition.opacity @keydown.escape.window="openDispo=false"
                                            style="display:none">
                                            <div class="min-h-full flex items-center justify-center p-4">
                                                <div class="w-full max-w-md rounded-2xl bg-white shadow-xl border border-slate-200"
                                                    x-show="openDispo" x-transition.scale>
                                                    <div class="px-5 py-4 rounded-t-2xl bg-slate-50 border-b">
                                                        <h3 class="font-semibold">Disposisi: {{ $row->no_tiket }}</h3>
                                                    </div>
                                                    <form method="post"
                                                        action="{{ route('upt.disposisi.store', $row) }}"
                                                        class="p-5 space-y-4">
                                                        @csrf
                                                        <label class="block text-sm">
                                                            <span class="mb-1 block text-slate-700">Pilih User
                                                                Layanan</span>
                                                            <select name="admin_layanan_id"
                                                                class="w-full rounded-lg border px-3 py-2" required>
                                                                <option value="">-- Pilih --</option>
                                                                @foreach ($userLayanan as $u)
                                                                    <option value="{{ $u->id }}">
                                                                        {{ $u->layanan?->nama }} – {{ $u->name }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
                                                            @if ($userLayanan->isEmpty())
                                                                <p class="mt-1 text-xs text-slate-500">Belum ada user
                                                                    layanan pada unit ini.</p>
                                                            @endif
                                                        </label>
                                                        <label class="block text-sm">
                                                            <span class="mb-1 block text-slate-700">Catatan
                                                                (opsional)
                                                            </span>
                                                        ...
                                                        <div class="flex items-center justify-end gap-2 pt-2">
                                                            <button type="button" @click="openDispo=false"
                                                                class="px-3 py-2 rounded-lg border border-slate-300 hover:bg-slate-50">Batal</button>
                                                            <button
                                                                class="cursor-pointer px-3 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white">Kirim
                                                                Disposisi</button>
                                                        </div>
                                                    </form>
                                                </div>
                                            ...
                                            <div class="min-h-full flex items-center justify-center p-4">
                                                <div class="w-full max-w-xl rounded-2xl bg-white shadow-xl border border-slate-200"
                                                    ...
                                                    <form method="post" enctype="multipart/form-data"
                                                        action="{{ route('upt.disposisi.jawab', $row) }}"
                                                        class="p-5 space-y-4">
                                                        @csrf
                                                        <label class="block text-sm">
