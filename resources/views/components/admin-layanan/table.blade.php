    ...
    <table class="min-w-full border-collapse text-sm">
        ...
        <thead class="bg-slate-100">
            <tr class="text-slate-700 font-semibold">
                <th class="border border-slate-200 px-4 py-3 text-left align-middle">UPT</th>
                <th class="border border-slate-200 px-4 py-3 text-left align-middle">Jenis Layanan</th>
                    ...
                    <td class="border border-slate-200 px-4 py-3 text-left align-middle whitespace-nowrap">
                        <div class="inline-flex items-center gap-2 flex-nowrap">
                            ...
                            <a href="{{ route('admin-layanan.show', $user->id) }}"
                                class="inline-flex items-center gap-1.5 px-3 py-1 text-xs rounded-md border border-indigo-300 text-indigo-700 hover:bg-indigo-50 transition font-medium">
                            ...
                            {{-- Edit --}}
                            <a href="{{ route('admin-layanan.edit', $user->id) }}"
            ...
            @empty
                <tr>
                    <td colspan="6" class="border border-slate-200 px-4 py-6 text-center text-slate-500 text-sm">
                        Tidak ada data admin Layanan ditemukan.
                    </td>
                </tr>
            @endforelse
        </tbody>
...
<script>
    function confirmNonaktif(id, name) {
        Swal.fire({
            ...
            html: `<p class="text-slate-600 text-sm">Akun <b>${name}</b> akan dinonaktifkan.</p>`,
    ...
    function confirmAktifkan(id, name) {
        Swal.fire({
            ...
            html: `<p class="text-slate-600 text-sm">Akun <b>${name}</b> akan diaktifkan kembali.</p>`,
            showCancelButton: true,
